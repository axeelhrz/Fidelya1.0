...
export function useEnhancedNotifications(): UseEnhancedNotificationsReturn {
  ...
  const loadTemplates = useCallback(async () => {
    try {
      ...
      const normalizedTemplates = templatesData.map((t: NotificationTemplate) => ({
        ...t,
        content: t.content ?? {},
  ...
  const loadQueueStats = useCallback(async () => {
    try {
      ...
      const queueData = await NotificationQueueService.getQueueStats() as unknown as QueueStats;
  ...
  const getCampaignAnalytics = useCallback(async (campaignId: string) => {
    try {
      return await NotificationAnalyticsService.getCampaignMetrics(campaignId);
    } catch (err) {
  ...
  const previewTemplate = useCallback(async (templateId: string, variables: Record<string, any>) => {
    try {
  ...
  const validateTemplate = useCallback(async (template: Partial<NotificationTemplate>) => {
    try {
      ...
      if (!template.content) errors.push('El contenido es requerido');
  ...
  return useMemo(() => ({
    ...
    loadingCampaigns,
    createCampaign,
    updateCampaign: async () => {}, // TODO: Implement
    deleteCampaign: async () => {}, // TODO: Implement
    sendCampaign,
    scheduleCampaign: async () => {}, // TODO: Implement
