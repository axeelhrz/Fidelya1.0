...
const isEndingSoon = (fechaFin: any) => {
  const vencimiento = fechaFin.toDate ? fechaFin.toDate() : new Date(fechaFin);
  const en7Dias = addDays(new Date(), 7);
  return isBefore(vencimiento, en7Dias);
};

const isNew = (fechaCreacion: any) => {
  const creacion = fechaCreacion.toDate ? fechaCreacion.toDate() : new Date(fechaCreacion);
...
export default function SocioBeneficiosPage() {
  ...
  const stats = useMemo<BenefitStats>(() => {
    ...
    const ahorroEsteMes = beneficiosUsados
      .filter(uso => {
        const fechaUso = uso.fechaUso.toDate ? uso.fechaUso.toDate() : new Date(uso.fechaUso);
  ...
  const filteredBeneficios = useMemo(() => {
    let filtered = beneficios.filter(beneficio => {
    ...
    filtered.sort((a, b) => {
      switch (filters.ordenar) {
        case 'fecha_desc':
          return (b.creadoEn.toDate ? b.creadoEn.toDate() : new Date(b.creadoEn)).getTime() - 
                 (a.creadoEn.toDate ? a.creadoEn.toDate() : new Date(a.creadoEn)).getTime();
        case 'fecha_asc':
          return (a.creadoEn.toDate ? a.creadoEn.toDate() : new Date(a.creadoEn)).getTime() - 
                 (b.creadoEn.toDate ? b.creadoEn.toDate() : new Date(b.creadoEn)).getTime();
        case 'descuento_desc':
          return b.descuento - a.descuento;
        case 'descuento_asc':
          return a.descuento - b.descuento;
        case 'vencimiento':
          return (a.fechaFin.toDate ? a.fechaFin.toDate() : new Date(a.fechaFin)).getTime() - 
                 (b.fechaFin.toDate ? b.fechaFin.toDate() : new Date(b.fechaFin)).getTime();
        case 'popularidad':
  ...
  if (error) {
    return (
      <DashboardLayout activeSection="beneficios" sidebarComponent={SocioSidebar}>
                    ...
                    <div className={cn('p-6', viewMode === 'list' && 'flex-1')}>
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
                        <div className="flex items-center gap-1">
                          <Calendar size={14} />
                          Vence: {format(
                            beneficio.fechaFin.toDate ? beneficio.fechaFin.toDate() : new Date(beneficio.fechaFin), 
                            'dd/MM/yyyy', 
                            { locale: es }
                          )}
                        </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
                      <div className="flex items-center gap-1">
                        <Calendar size={14} />
                        {format(
                          uso.fechaUso.toDate ? uso.fechaUso.toDate() : new Date(uso.fechaUso), 
                          'dd/MM/yyyy HH:mm', 
                          { locale: es }
                        )}
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-xl border border-blue-200">
                      ...
                      <p className="text-lg font-bold text-blue-700">
                        {format(
                          selectedBenefit.fechaFin.toDate ? selectedBenefit.fechaFin.toDate() : new Date(selectedBenefit.fechaFin), 
                          'dd MMMM yyyy', 
                          { locale: es }
                        )}
                      </p>
