...
export const BeneficiosList: React.FC<BeneficiosListProps> = ({
  ...
  const getOrigenIcon = (beneficio: Beneficio) => {
    const origen = (beneficio as any).origenBeneficio;
    
    switch (origen) {
      case 'asociacion':
        return <Users className="w-4 h-4 text-purple-600" title="Beneficio de tu asociación" />;
      case 'comercio_vinculado':
        return <Building2 className="w-4 h-4 text-blue-600" title="Comercio vinculado a tu asociación" />;
      case 'comercio_afiliado':
        return <Building2 className="w-4 h-4 text-green-600" title="Comercio afiliado" />;
      case 'publico':
        return <Gift className="w-4 h-4 text-orange-600" title="Beneficio público" />;
      case 'directo':
        return <Star className="w-4 h-4 text-yellow-600" title="Acceso directo" />;
      default:
