...
class NotificationApprovalService {
    ...
    try {
      ...
      requests.forEach(request => {
        ...
        if (request.completedAt && (request.status === 'approved' || request.status === 'rejected')) {
          const approvalTime = request.completedAt.toMillis() - request.createdAt.toMillis();
      ...
      if (stats.totalRequests > 0) {
        stats.approvalRate = (stats.approvedRequests / stats.totalRequests) * 100;
      }
  ...
  // Métodos privados
  private async findApplicableWorkflow(notificationData: any): Promise<ApprovalWorkflow | null> {
    try {
  ...
  private matchesTriggers(notificationData: any, triggers: WorkflowTrigger[]): boolean {
    return triggers.every(trigger => {
      const value = notificationData[trigger.type];
      ...
      switch (trigger.operator) {
        ...
        case 'less_than':
          return Number(value) < Number(trigger.value);
        case 'in':
          return Array.isArray(trigger.value) && trigger.value.includes(value);
        default:
          return false;
      }
    });
