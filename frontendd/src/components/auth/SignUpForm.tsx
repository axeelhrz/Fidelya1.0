...
// Esquema completo para el paso 2
...
const SignUpForm: React.FC = () => {
  ...
  const {
    ...
    trigger,
    setValue,
  } = useForm<RoleFormData | CompleteFormData>({
  ...
  const onSubmit = async (data: RoleFormData | CompleteFormData) => {
    if (currentStep === 0) {
    ...
    const completeData = data as CompleteFormData;
    
    // Validar campos específicos según el rol
    const validationErrors: string[] = [];
    
    if (selectedRole === 'liga') {
      if (!completeData.league_name?.trim()) validationErrors.push('El nombre de la liga es requerido');
      if (!completeData.province?.trim()) validationErrors.push('La provincia es requerida');
    } else if (selectedRole === 'club') {
      if (!completeData.club_name?.trim()) validationErrors.push('El nombre del club es requerido');
      if (!completeData.league_id) validationErrors.push('Selecciona una liga');
      if (!completeData.city?.trim()) validationErrors.push('La ciudad es requerida');
      if (!completeData.address?.trim()) validationErrors.push('La dirección es requerida');
    } else if (selectedRole === 'miembro') {
      if (!completeData.full_name?.trim()) validationErrors.push('El nombre completo es requerido');
      if (!completeData.club_id) validationErrors.push('Selecciona un club');
      if (!completeData.birth_date) validationErrors.push('La fecha de nacimiento es requerida');
      if (!completeData.gender) validationErrors.push('Selecciona el sexo');
      if (!completeData.rubber_type) validationErrors.push('Selecciona el tipo de caucho');
    }
  ...
  const getFieldError = (fieldName: keyof CompleteFormData) => {
    if (currentStep === 0) return undefined;
    return (errors as any)[fieldName];
  };

  // Función helper para obtener valores de campos específicos
  const getFieldValue = (fieldName: keyof CompleteFormData) => {
    return (watchedFields as any)[fieldName];
  };
